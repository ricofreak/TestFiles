...
<xsl:stylesheet version="1.0"
  ...
  exclude-result-prefixes="marc items str">
    <xsl:import href="https://raw.githack.com/ricofreak/TestFiles/master/testUtils.xsl"/>
                    ...
                    <xsl:for-each select="marc:subfield[contains('bcfghknps', @code)]">
                        <xsl:choose>
                            <xsl:when test="@code='b'">
                                <!--  13381 Span class around subfield b so it can be suppressed via css -->
                                <span class="title_subtitle"><xsl:apply-templates/><xsl:text>&#160;</xsl:text> </span>
                            </xsl:when>
    ...
    <xsl:if test="$DisplayIconsXSLT!='0' and $materialTypeCode!=''">
        ...
        <xsl:for-each select="marc:datafield[@tag=490]">
            <a><xsl:attribute name="href">/cgi-bin/koha/catalogue/search.pl?q=se,phr:"<xsl:call-template name="chopPunctuation"><xsl:with-param name="chopString"><xsl:value-of select="str:encode-uri(marc:subfield[@code='a'], true())"/></xsl:with-param></xsl:call-template>"</xsl:attribute>
                        <xsl:call-template name="chopPunctuation">
                            <xsl:with-param name="chopString">
                                <xsl:call-template name="subfieldSelect">
                                    <xsl:with-param name="codes">av</xsl:with-param>
